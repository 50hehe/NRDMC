...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def __init__(self, config, model, mg=False):
        ...
        # ------------------------------------
        self.mg = mg
        self.loss_part_names = ['rec_loss', 'align_loss', 'multi_view_loss']
        self.train_loss_parts = {}  # {epoch_idx: {'rec_loss': x, ...}}
        # ------------------------------------
    ...
    def _generate_train_loss_output(self, epoch_idx, s_time, e_time, losses):
        head = f'epoch {epoch_idx} training [time: {e_time - s_time:.2f}s, '
        if isinstance(losses, tuple):
            per = ', '.join(f'{name}: {val:.4f}'
                            for name, val in zip(self.loss_part_names, losses))
            total = float(sum(losses))
            return head + per + f', total: {total:.4f}]'
        else:
            return head + f'train loss: {float(losses):.4f}]'
    ...
    def fit(self, train_data, valid_data=None, test_data=None, saved=False, verbose=True):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if isinstance(train_loss, tuple):
                # 保存各项 & 总损失（总损失直接 sum 即可）
                self.train_loss_parts[epoch_idx] = dict(
                    zip(self.loss_part_names, [float(x) for x in train_loss])
                )
                self.train_loss_dict[epoch_idx] = float(sum(train_loss))
            else:
                self.train_loss_dict[epoch_idx] = float(train_loss)
            # -------------------------------------------------------------------------------------------------

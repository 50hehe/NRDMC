...
def build_sim(context): # [N, D]
    context_norm = context.div(torch.norm(context, p=2, dim=-1, keepdim=True)) # [N, 1] [N, N]
...
def build_knn_normalized_graph(adj, topk, is_sparse, norm_type):
    device = adj.device
    knn_val, knn_ind = torch.topk(adj, topk, dim=-1) # [N, k] 每行前k大的相似度, [N, k] 对应的列索引
    if is_sparse:
        ...
        return torch.sparse_coo_tensor(edge_index, edge_weight, adj.shape) # 返回coo格式的稀疏张量
    else:
        weighted_adjacency_matrix = (torch.zeros_like(adj)).scatter_(-1, knn_ind, knn_val)
        return get_dense_laplacian(weighted_adjacency_matrix, normalization=norm_type) # 返回稠密矩阵
...
class NEGCL(GeneralRecommender):
    def __init__(self, config, dataset):
        super(NEGCL, self).__init__(config, dataset)
    ...
    def forward(self, perturbed=False):
        ...
        if self.v_feat is not None and self.t_feat is not None:
            ...
            mm_emb = self.beta * F.normalize(ghe_embs) + mcl_embs

            all_embs = str_emb + mm_emb
        else:
            all_embs = inter_embs
    ...
    def calculate_loss(self, interaction, sign):
        ua_embeddings, ia_embeddings, hyper_embeddings, mm_embeddings = self.forward(sign)
        users = interaction[0]

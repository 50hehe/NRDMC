...
class HPMREC(GeneralRecommender):
    def __init__(self, config, dataset):
        super(HPMREC, self).__init__(config, dataset)
        ...
        self.algebra_dim = config['algebra_dim']
        ...
        if self.v_feat is not None:
            self.v_gcn = HypercomplexGNN(num_user=num_user, num_item=num_item, embed_dim=self.dim_latent,
                                         ...
                                         device=self.device, features=self.v_feat, algebra_dim=self.algebra_dim)

        if self.t_feat is not None:
            self.t_gcn = HypercomplexGNN(num_user=num_user, num_item=num_item, embed_dim=self.dim_latent,
                                         num_layers=self.num_layers,
                                         device=self.device, features=self.t_feat, algebra_dim=self.algebra_dim)
    ...
    def decompose_hypercomplex(self, hyper_embed):
        ...
        real = chunks[0]
        imaginary = torch.mean(torch.stack(chunks[1:], dim=1), dim=1)  # 虚部联合分量的均值
...
class HypercomplexGNN(torch.nn.Module):
    ...
    def forward(self, edge_index, features):
        ...
        for n in range(self.num_layers):
            linear_outputs = []
            for i in range(self.algebra_dim):
                features_out = self.gnn_layers[i](h_components[i], edge_index)  # 使用对应的 gnn_layer 进行消息传递
        ...
        linear_total = torch.sum(torch.stack(linear_total, dim=0), dim=0)  # [num_nodes, algebra_dim * dim_latent]
...
class CayleyDicksonMult(nn.Module):
    ...
    def forward(self, h1, h2):
        if self.algebra_dim == 2:
            real1, imag1 = torch.chunk(h1, 2, dim=1)  # [*, dim/2]
            real2, imag2 = torch.chunk(h2, 2, dim=1)  # [*, dim/2]
            ...
            a, b = torch.chunk(h1, 2, dim=1)  # [*, dim/2], [*, dim/2]
            c, d = torch.chunk(h2, 2, dim=1)  # [*, dim/2], [*, dim/2]
